...
export const IntegrationPanel = (integration: {
  name: string;
  workflows: Array<any>;
  addCheckedAction: (action: any) => void;
  removeCheckedAction: (actionName: string) => void;
  activeTools: any;
}) => {
  ...
  const fetchActions = async () => {
    const response = await fetch(
      "https://actionkit.useparagon.com/projects/" +
    ...
    if (paragonUser?.authenticated) {
      await fetch(backend + "/api/trigger/gdrive-filepicker", {
        ...
        .then((response) => {
          response.json();
          toast.success(
            "Files picked for ingestion! It will take a moment to retrieve files",
            {
              position: "top-right",
            },
          );
        })
        .catch((error) => console.log("Error trigger gdrive: " + error));
    }
  }
  ...
  return (
    <div className="flex flex-col items-center max-h-96 overflow-y-scroll">
      <div className="flex w-full">
        <button
          onClick={() => {
            setPanelState((prev) => ({ ...prev, active: "actions" }));
          }}
          className={
            panelState.active === "actions"
              ? "font-bold border-stone-300 bg-stone-300 border-t-2 basis-1/2 rounded-t-sm border-x-2"
              : "bg-stone-100 border-t-2 basis-1/2 rounded-t-sm border-x-2"
          }
        >
          Actions
        </button>
        <button
          onClick={() => {
            setPanelState((prev) => ({ ...prev, active: "workflows" }));
          }}
          className={
            ...
            return (
              ...
              >
                <input
                  ...
                  value={action.function.name}
                  checked={
                    action.function.name in integration.activeTools
                      ? true
                      : false
                  }
                />
                <div>{parseNames(action.function.name)}</div>
              </div>
            );
          })}
        {panelState.active === "workflows" && (
          <>
            ...
            {integration.name === "googledrive" && (
              <button
                ...
                className="bg-blue-700 text-white w-fit place-self-center rounded-md py-1 px-2 text-sm hover:bg-blue-500"
              >
                Open Filepicker
              </button>
            )}
